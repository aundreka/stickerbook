...
// Logs via console.warn (console.error is stripped at build) and surfaces a
// banner only if boot never produced a canvas.
...
// Per-frame poll handles rotation/resize uniformly across WebViews.
function poll(): void {
  const { w, h } = viewportSize()
...
function bindResize(): void {
  ...
  window.addEventListener('orientationchange', () => {
    debounced()
    // mobile browsers settle viewport metrics late after rotation
